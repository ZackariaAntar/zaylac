		...
		<View className="flex-1 bg-primary">
			...
			<ScrollView contentContainerStyle={{ padding: 16 }}>
				...
				<View className="flex-row justify-between items-center mb-2">
					<Text className="text-white-100 font-pbold text-xl font-bold">
						Account Info
					</Text>
					<TouchableOpacity
				...
				<View className=" m-4 bg-teal-700  p-2 rounded-lg">
					<Text className="font-pbold text-white-100 text-md mb-1">
						Username
					</Text>
					<TextInput
						className={`bg-gray-100 w-[85%] text-lg text-gray-900 px-3 py-2 rounded mb-4 ${
							isEditing ? "" : "opacity-75 bg-transparent"
						}`}
						value={username}
					...
					<TextInput
						...
						placeholder={`${accountInfo.phone_number}`}
						editable={isEditing}
					/>
				</View>
				...
				<View>
					<View className="flex-row justify-between items-center mb-4">
						<Text className="text-white-100 text-xl font-bold">
							Payment Methods
						</Text>
						<TouchableOpacity
					...
					{cards.map((card, index) => (
						<View
							key={index}
							className="bg-gray-400 h-auto w-full rounded-lg relative mb-4 p-4"
						>
							{editingCardIndex === index ? (
								...
								<>
									<Text className="text-white-100 text-lg mb-1">
										{card.name}
									</Text>
									<Text className="text-white-100 text-lg mb-1">
										**** **** **** {card.number.slice(-4)}
